...
import type { AdminConfig, ModelConfig } from "../config/index.ts";

export const generateRoutes = (config: AdminConfig): Record<string, PipeFn> => {
  const routes: Record<string, PipeFn> = {};
  ...
  // Schema endpoint
  routes[`GET ${base}/api/schema`] = withAuth(
    pipe((c) => {
  ...
  for (const modelCfg of config.models) {
    ...
    if (!modelCfg.readOnly) {
      ...
      // DELETE
      routes[`DELETE ${prefix}/:id`] = withAuth(
        pipe(async (c) => {
      ...
      // BULK
      routes[`POST ${prefix}/bulk`] = withAuth(
        pipeline(parseJson)(
          pipe(async (c) => {
    ...
    if (modelCfg.actions?.length) {
      routes[`POST ${prefix}/action`] = withAuth(
        pipeline(parseJson)(
          pipe(async (c) => {
            const { action, ids } = c.body as { action: string; ids: (string | number)[] };
    ...
    if (modelCfg.relations?.length) {
      for (const rel of modelCfg.relations) {
        routes[`GET ${prefix}/:id/relations/${rel.schema.table}`] = withAuth(
          pipe(async (c) => {
