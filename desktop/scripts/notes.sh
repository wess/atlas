...
# section this exits non-zero so the caller can fall back rather than publish an
# empty release body.
#
...
# From the "## <version>" heading to the line before the next "## ". Blank
# lines *inside* the section are content — they separate paragraphs — so only
# the leading and trailing runs are trimmed.
